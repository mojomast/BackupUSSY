...
class DatabaseManager:
    ...
    def add_tape(self, tape_label: str, device: str, notes: Optional[str] = None, tape_status: str = 'active') -> int:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                now = datetime.now()
                cursor.execute("""
                    INSERT INTO tapes (tape_label, tape_device, created_date, last_written, notes, tape_status)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (tape_label, device, now, now, notes, tape_status))
    ...
    def find_archive_by_name(self, archive_name: str) -> Optional[Dict[str, Any]]:
        """Find an archive by its name.
        
        Args:
            archive_name: Name to search for
            
        Returns:
            Archive record or None if not found
        """
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                ...
                cursor.execute("SELECT * FROM archives WHERE archive_name = ?", (archive_name,))
                result = cursor.fetchone()
                
                return dict(result) if result else None
                
        except sqlite3.Error as e:
            logger.error(f"Failed to find archive by name: {e}")
            return None
    
    def get_archive_files(self, archive_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to get archive files: {e}")
            return []

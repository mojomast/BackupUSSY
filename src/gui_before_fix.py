...
class LTOArchiveGUI:
    ...
    def create_archive_tab(self):
        ...
        device_frame = [
            [sg.Text('Tape Device:', size=(12, 1)),
             sg.Combo(
                 devices, default_value=devices[0], key='-DEVICE-', size=(20, 1)),
        ...
        progress_frame = [
            [sg.Text('Progress:'), sg.ProgressBar(
                100, key='-PROGRESS-', size=(50, 20))],
            [sg.Text('Status:', size=(8, 1)), sg.Text(
                'Ready', key='-STATUS-', size=(60, 1))],
            [sg.Multiline(size=(100, 12), key='-LOG-',
        ...
        return [
            ...
            [sg.Frame('Progress', progress_frame,
                      expand_x=True, expand_y=True)],
    ...
    def create_recovery_tab(self):
        ...
        recovery_progress_frame = [
            ...
            [sg.Text('Status:', size=(8, 1)), sg.Text(
                'Ready', key='-RECOVERY_STATUS-', size=(60, 1))],
            [sg.Multiline(size=(100, 8), key='-RECOVERY_LOG-',
                          disabled=True, autoscroll=True)]
        ]
        ...
        recovery_controls_frame = [
            [sg.Button('Start Recovery', key='-START_RECOVERY-', size=(12, 1), disabled=True),
             sg.Button('Cancel Recovery', key='-CANCEL_RECOVERY-',
                       size=(12, 1), disabled=True),
        ...
        return [
            ...
            [sg.Frame('File Selection', file_list_frame,
                      expand_x=True, expand_y=True)],
    ...
    def create_search_tab(self):
        ...
        search_controls_frame = [
            [sg.Button('Recover Selected', key='-SEARCH_RECOVER-', size=(12, 1), disabled=True),
             sg.Button('Show in Archive', key='-SEARCH_SHOW_ARCHIVE-',
                       size=(12, 1), disabled=True),
        ...
        return [
            ...
            [sg.Frame('Search Results', search_results_frame,
                      expand_x=True, expand_y=True)],
    ...
    def create_management_tab(self):
        ...
        library_stats_frame = [
            ...
            [sg.Text('Total Tapes:', size=(15, 1)),
                     sg.Text('0', key='-STAT_TOTAL_TAPES-')],
            [sg.Text('Active Tapes:', size=(15, 1)),
                     sg.Text('0', key='-STAT_ACTIVE_TAPES-')],
            [sg.Text('Total Archives:', size=(15, 1)),
                     sg.Text('0', key='-STAT_TOTAL_ARCHIVES-')],
            [sg.Text('Total Files:', size=(15, 1)),
                     sg.Text('0', key='-STAT_TOTAL_FILES-')],
            [sg.Text('Total Capacity:', size=(15, 1)),
                     sg.Text('0 GB', key='-STAT_TOTAL_SIZE-')]
        ]
        ...
        tape_list_frame = [
            ...
            [sg.Table(
                values=[],
                headings=['Label', 'Device', 'Status',
                    'Size', 'Archives', 'Created'],
                key='-TAPE_LIST-',
                auto_size_columns=False,
        ...
        tape_details_frame = [
            ...
            [sg.Text('Label:', size=(8, 1)), sg.Input(
                key='-TAPE_DETAIL_LABEL-', size=(20, 1), disabled=True)],
            ...
            [sg.Multiline(key='-TAPE_DETAIL_NOTES-',
                          size=(60, 4), disabled=True)]
        ]

        # Management controls
        manage_controls_frame = [
            [sg.Button('Edit Tape', key='-EDIT_TAPE-', size=(12, 1), disabled=True),
             sg.Button('Delete Tape', key='-DELETE_TAPE-',
                       size=(12, 1), disabled=True),
        ...
        right_column = [
            [sg.Frame('Tape Inventory', tape_list_frame,
                      expand_x=True, expand_y=True)]
        ]
    ...
    def initialize_managers(self):
        ...
        try:
            ...
            sg.popup_error('Initialization Error',
    ...
    def start_archive_job(self, config):
        ...
        self.log_message(
            f"Archive job started for: {config.get('archive_name', 'Unnamed Archive')}")
    ...
    def pre_archive_checks(self, config):
        ...
        if not folder_path or not os.path.exists(folder_path):
            sg.popup_error(
                'Error', 'Source folder does not exist or is not selected')
            return False
        ...
        try:
            ...
            sg.popup_error(
                'Error', 'Permission denied accessing source folder')
            return False
    ...
    def run_archive_job(self, config):
        ...
        try:
            ...
            # This logic was moved from the orphaned finally block
            ...
            if 'job_result_for_event_loop' in locals() and isinstance(job_result_for_event_loop, dict):
                ...
                if hasattr(self, 'logger') and self.logger:
                    self.logger.error(
                        "CRITICAL: job_result_for_event_loop was not defined by try/except in run_archive_job. Sending default error.")
                result_to_send = {
                    'success': False,
                    'error_message': 'Job result was not properly generated due to a critical internal error.',
                    'archive_id': config.get('archive_id'),
                    'files_processed': 0,
                    'bytes_written': 0,
                    'mode': config.get('mode'),
    ...
    def handle_second_copy_cached(self, archive_path, device, progress_callback):
        ...
        if result == 'Yes':
            ...
            if success:
                self.log_message(
                    f"Second copy completed successfully ({bytes_written} bytes)")
            else:
                self.log_message("Second copy failed")
        ...
        while True:
            ...
            if event == sg.WIN_CLOSED or event == '-EXIT-':
                if self.job_thread and self.job_thread.is_alive():
                    ...
                    self.log_message(
                        "Attempting to cancel active job before exiting...", 'warn')
            ...
            if event == '-START-':
                ...
                if values['-FOLDER-']:
                    ...
                    sg.popup(
                        'Please select a folder first to calculate its size.')

            elif event == '-REFRESH-':  # Refresh tape devices
                devices = self.tape_manager.detect_tape_devices()
                if not devices:
                    devices = [r'\.\Tape0']  # Default
                ...
                self.log_message(
                    "Cancel button clicked - cancellation logic to be implemented.", 'warn')
                if self.current_job:
            ...
            # --- Recovery Tab Events ---
                ...
                self.handle_recovery_tape_selection(
                    values.get('-RECOVERY_TAPE-'))
            elif event == '-LOAD_TAPE-':
                self.handle_load_tape_contents(values.get('-RECOVERY_TAPE-'))
            elif event == '-RECOVERY_ARCHIVE-':
                self.handle_recovery_archive_selection(
                    values.get('-RECOVERY_ARCHIVE-'))
            ...
            # --- JOB DONE Event --- (From run_archive_job thread)
                ...
                if job_result:
                    ...
                    archive_name_msg = f" for archive '{job_result.get('archive_name', 'N/A')}'" if job_result.get(
                        'archive_name') else ""
                    self.log_message(
                        f"Archive job finished with status: {status}{archive_name_msg}")

                    if job_result.get('error_message'):
                        self.log_message(
                            f"Error: {job_result['error_message']}", 'error')
                        sg.popup_error(
                            f"Job Failed: {job_result['error_message']}")
                    elif status == 'completed' or status == 'indexed':
                        self.log_message(
                            f"Successfully processed {job_result.get('files_processed', 0)} files, {job_result.get('bytes_written', 0)} bytes written.")
                    elif status == 'streaming_failed' or status == 'caching_failed':
                         self.log_message(
                             f"Job failed. See logs for details.", 'error')
                    else:
                        # Generic completion
                        self.log_message("Job completed.")
                    ...
                    self.log_message(
                        "Job finished, but no result data was received.", 'warn')
...
def handle_preview_files(self, folder_path):
        ...
        try:
            ...
            sg.popup_scrolled(
                preview_text, title='File Preview', size=(80, 30))
        except Exception as e:
...
def handle_recovery_tape_selection(self, tape_selection):
        ...
        try:
            ...
            if tape:
                ...
                archive_options = [archive['archive_name']
                    for archive in archives]
                self.window['-RECOVERY_ARCHIVE-'].update(
                    values=archive_options)
        except Exception as e:
...
Active: {stats.get('active_tapes', 0)}"""
        ...
        try:
            ...
            for tape in tapes:
                ...
                table_data.append([
                    ...
                    f"{size_gb:.1f} GB",
                    str(len(archives)),
                    tape.get('created_date', 'Unknown')[:10] if tape.get(
                        'created_date') else 'Unknown'
                ])
    ...
    def handle_add_tape(self):
            ...
            layout = [
                ...
                [sg.Text('Device:', size=(12, 1)), sg.Input(
    ...
    def handle_db_maintenance(self):
        ...
        try:
            ...
            sg.popup_error('DB Maintenance Error', f'An error occurred in maintenance window: {e}')
        finally:
            if 'maint_window' in locals() and maint_window: # Check if window was successfully created
                maint_window.close()
    ...
    try:
        ...
        if not selected_rows:
            ...
            if all_results_metadata is None:
                ...
                sg.popup_error("Error", "Cannot retrieve archive details: table metadata missing.")
                return

            all_results = all_results_metadata.get('results', [])
            if not all_results:
                self.logger.warning("No results found in search table metadata.")
                sg.popup_error("Error", "Cannot retrieve archive details: no results in metadata.")
                return

            if not selected_rows or selected_rows[0] >= len(all_results):
                self.logger.warning(f"Invalid row selection: {selected_rows} for results length {len(all_results)}.")
                sg.popup_error("Error", "Invalid file selection to show archive.")
                return
            
            selected_file_data = all_results[selected_rows[0]]
            
            # Assuming archive_name is at index 3 of selected_file_data
            # Ensure selected_file_data is a list/tuple and has enough elements
            if isinstance(selected_file_data, (list, tuple)) and len(selected_file_data) > 3:
                archive_name = selected_file_data[3]
            else:
                self.logger.error(f"Selected file data is not in expected format or too short: {selected_file_data}")
                sg.popup_error("Error", "Could not retrieve archive name from selected file data.")
                return
...
def handle_search_export(self):
    ...
    try:
        ...
        if not table_data:
            ...
            if not export_path:
                sg.popup_cancelled('Export cancelled by user.')
                return
